...
# Compteur RGT ------------------------------------------------Compteur pour RGT par Jalon------------------------


	ETD.rechercher("select regroupement_rgt , count(distinct jalon)  from C_OK_RGT where Jalon in ('Piquetage non réalisé', 'Piquetage', 'Etude', 'Travaux lancés', 'DOE') GROUP by regroupement_RGT having count(distinct jalon) = 1 ")
	a = ETD.resultat()

	for i, j in a :

		ETD.rechercher("update C_OK_RGT set regroupement_rgt = " + "'" + "RGT-" + i + "_" + '01' +  "'" + " where regroupement_rgt = " "'" + i + "'")
		

	ETD.rechercher("select regroupement_rgt, Jalon, count(distinct jalon)  from C_OK_RGT where Jalon in ('Piquetage non réalisé', 'Piquetage', 'Etude', 'Travaux lancés', 'DOE') GROUP by regroupement_RGT having count(distinct jalon) > 1")
	a = ETD.resultat()

	for i, j, k in a :

		ETD.rechercher("SELECT  distinct Jalon from C_OK_RGT where regroupement_rgt = " + "'" + i + "'" + " and Jalon in ('Piquetage non réalisé', 'Piquetage', 'Etude', 'Travaux lancés', 'DOE')")
		b = ETD.resultat()
		cpt = 1
		for k in b:

			ETD.rechercher("update C_OK_RGT set regroupement_rgt = " + "'" + "RGT-" + i + "_" + '0' + str(cpt) +  "'" + " where regroupement_rgt = " "'" + i + "'" + " and Jalon = " "'" + k[0] + "'" )
			cpt = cpt + 1
