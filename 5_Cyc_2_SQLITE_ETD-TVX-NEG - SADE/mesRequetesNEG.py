...
table_OPT = {
				...
				49 :   ['Regroupement_ZN', 'Reg_Nego', 'CHARINT', 'liste[49]', '?'],
				50 :   ['Regroupement_ZE', 'Regroupement_ZE', 'CHARINT', 'liste[50]', '?'],
				51 :   ['Regroupement_RGT', 'Regroupement_RGT', 'CHARINT', 'liste[50]', '?'],
				52 :   ['Numero_FCI_Acces', 'Numero_FCI_Acces', 'CHARINT', 'liste[50]', '?'],
				53 :   ['Numero_FCI_Fin', 'Numero_FCI_Fin', 'CHARINT', 'liste[50]', '?'],
				54 :   ['PA_INI', 'PA_INI', 'CHARINT', 'liste[9]', '?'],
				55 :   ['Statut_aff_Syndic', 'Statut affectation syndic', 'CHARINT', 'liste[51]', '?'],
				56 :   ['Type_DTA', 'Type DTA', 'CHARINT', 'liste[52]', '?'],
				57 :   ['Nego_non_deb', 'Nego_non_deb', 'CHARINT', 'liste[53]', '?'],
				58 :   ['Qualifie', 'Qualifie', 'CHARINT', 'liste[54]', '?'],
				59 :   ['ImportZN', 'ImportZN', 'CHARINT', 'liste[55]', '?'],
				60 :   ['PrevAccord', 'PrevAccord', 'CHARINT', 'liste[56]', '?'],
				61 :   ['Accord', 'Accord', 'CHARINT', 'liste[57]', '?'],
				62 :   ['DTA', 'DTA', 'CHARINT', 'liste[58]', '?'],
				63 :   ['BPE', 'BPE', 'CHARINT', 'liste[59]', '?'],
				64 :   ['Piquetage', 'Piquetage', 'CHARINT', 'liste[60]', '?'],
				65 :   ['BPT', 'BPT', 'CHARINT', 'liste[61]', '?'],
				66 :   ['Etudes', 'Etudes', 'CHARINT', 'liste[62]', '?'],
				67 :   ['Travaux_lances', 'Travaux_lances', 'CHARINT', 'liste[63]', '?'],
				68 :   ['Travaux_realises', 'Travaux_realises', 'CHARINT', 'liste[64]', '?'],
				69 :   ['DOE', 'DOE', 'CHARINT', 'liste[65]', '?'],
				70 :   ['Jalon', 'Jalon', 'CHARINT', 'liste[66]', '?'],
				71 :   ['IMM_PAV', 'IMM_PAV', 'CHARINT', 'liste[67]', '?'],
				72 :   ['Regroupement', 'Regroupement', 'CHAR', 'liste[68]', '?'],
				73 :   ['centre_etude', 'centre_etude', 'CHAR', 'liste[69]', '?'],
				74 :   ['centre_travaux', 'centre_travaux', 'CHAR', 'liste[70]', '?'],
				75 :   ['fci_acces', 'Numéro FCI d’accès', 'CHAR', 'liste[71]', '?'],
				76 :   ['fci_fin', 'Numéro FCI fin de travaux', 'CHAR', 'liste[72]', '?'],
				77 :   ['date_fci', 'date FCI', 'INT', 'liste[73]', '?'],
				78 :   ['table_etd', 'table etude', 'CHAR', 'liste[74]', '?'],
				79 :   ['Suspendre', 'Suspendre', 'CHAR', 'liste[75]', '?'],
				80 :   ['Type_suspension', 'Type suspension', 'CHAR', 'liste[76]', '?'],
				81 :   ['SPI', 'Migré SPI', 'CHAR', 'liste[77]', '?']
...
updateEtudes = "update OPT set Etudes = 'Oui' where (Etat_Nego <> 'PAS DE SYNDIC' and Resultat_etude_site = 'OK' and Type_Site <> '' and NB_EL_P <> '' and NB_EL_R <> '') OR (Etat_Nego = 'PAS DE SYNDIC' and Resultat_etude_site = 'OK' and Type_Site <> '' and NB_EL_P <> '' and NB_EL_R <> '')" 
...
fichierNonQualif = "select Code_IMB, Adresse, Localite, CP, Code_Syndic, Nom_Syndic, NB_EL, Regroupement_ZN, Type_regroupement, '' Date_Realisation, '' Type_Syndic, Nom_contact_syndic, '' Type_contact, '' Adresse_Contact, Tel_contact_syndic, '' mail_contact_syndic, Etat_reseau_TD, Date_reseau_TD, '' Qualificateur  from C1_C2_OK where Qualifie || ImportZN || PrevAccord || Accord || BPE || BPT = 'NonNonNonNonNonNon' and Etat_Nego <> 'PAS DE SYNDIC'"

fichierQualif = "select Code_IMB, Adresse, Localite, CP, Code_Syndic, Nom_Syndic, NB_EL, Regroupement_ZN, Type_regroupement, '' Date_Realisation, '' Type_Syndic, Nom_contact_syndic, '' Type_contact, '' Adresse_Contact, Tel_contact_syndic, '' mail_contact_syndic, Etat_reseau_TD, Date_reseau_TD, '' Qualificateur  from C1_C2_OK where Qualifie || ImportZN || PrevAccord || Accord || BPE || BPT = 'OuiNonNonNonNonNon' and Etat_Nego <> 'PAS DE SYNDIC'"

fichierImportZN = "select Code_IMB, Adresse, Localite, CP, Code_Syndic, Nom_Syndic, NB_EL, Regroupement_ZN, Type_regroupement, '' Date_Realisation from C1_C2_OK where Qualifie || ImportZN || PrevAccord || Accord || BPE || BPT = 'NonOuiNonNonNonNon' and Etat_Nego <> 'PAS DE SYNDIC'"

fichierPrevAccord = "select Code_IMB, Adresse, Localite, CP, Code_Syndic, Nom_Syndic, NB_EL, Regroupement_ZN, Type_regroupement, '' Date_Realisation, Statut_aff_Syndic, AG_non_pertinente, Date_AG, Date_envoi_AS, '' site_en_copro, '' flag_imm_neuf from C1_C2_OK where Qualifie || ImportZN || PrevAccord || Accord || BPE || BPT in ('NonOuiOuiNonNonNon', 'OuiNonOuiNonNonNon') and Etat_Nego <> 'PAS DE SYNDIC'"
...
fichierDTA = "select Code_IMB, Adresse, Localite, CP, Code_Syndic, Nom_Syndic, NB_EL, Regroupement_ZN, Type_regroupement, '' Date_Realisation, Presence_DTA, Type_DTA from C1_C2_OK where (CRS in (select distinct CRS from C1_C2_OK group by CRS having count(Distinct DTA) = 1) and DTA = 'Oui') \
OR ( DTA = 'Oui' and CRS = '')"
...
Attente_Probation = " where Attente_Probation not in ('1-T/D1 non effectué','2-Attente DTA','3-Raccordement aérien','4-Imb -12logmts qu‘on ne sait pas produire','5-Accord Free','6-PME non déployé','7-Autres')"
