...
DPL_OPT = "select Code_IMB, Adresse, CP, Localite, '' plaque, ID_SYNDIC, NB_EL, '' PB, PA, PM, Date_envoi_PE_au_syndic, NB_escaliers, NB_EL_R, NB_EL_P, Type_de_site, Resultat_etude_d2, Date_pose_PB, Date_envoi_PE_au_syndic Date_MaD_PE, Etat_NEGO, Date_prog_racco, Nom_Syndic, Code_regroupement_syndic, Nom_contact, '' Type_de_regroupement, '' Etat_reseau_TD, '' Date_realisation_TD, AG_pertinente, '' Date_AG, Date_Envoi_Accord_Type, '' Date_retour_accord, Date_d_accord_syndic, Date_retour_convention, Attente_Probation, Date_de_validation_PE, Presence_DTA, Civilite_President, Nom_PdtConseilSyndical, Prenom_PdtConseilSyndical, N_telephone_pdtconseilsyndical, N_mobile_pdtconseilsyndical, N_fax_pdtconseilsyndical, '' mail_president, Prenom_Responsable, N_Tel_syndic, N_Mobile_Syndic, Date_reception_BPE, '' Date_souhaitee_PE, Date_fin_EZA, Regroupemen_ZN, Regroupement_ZE, Regroupement_RGT, Numero_FCI_acces, Numero_FCI_fin  from OPT where code_imb <> ''"


infos = "create table infos as select OPT.code_imb, Qualificateur, Nom_contact, Type_contact, Adresse_contact, Tel_contact, Mail_contact, Commentaire_Qualif, Civilite_President, Email_PdtConseilSyndical, N_fax_pdtconseilsyndical, Nom_PdtConseilSyndical, Prenom_PdtConseilSyndical, N_telephone_pdtconseilsyndical, N_mobile_pdtconseilsyndical, Civilite_Responsable, Email_Syndic, N_Fax_syndic, Nom_Responsable, Prenom_Responsable, N_Tel_syndic, N_Mobile_Syndic, Commentaire_ZN, Negociateur, PB_Geof_Geofibre, Adduction_pb_vers_PA, Type_qualif_ETD, Numero_support_site, Adduction_pb_vers_EL, PT_IPON, Commentaire_RGT, ID_ZN, Centre, '' date_action, plaque, IXI.Adresse, IXI.Localite \
from OPT \
left join IXI \
on OPT.Code_IMB = IXI.Code_IMB"

Qualificateur = "select code_IMB, qualificateur from infos where Qualificateur <> ''"

Negociateur = "select distinct ID_ZN, Negociateur from infos where Negociateur <> '' and ID_ZN is not null"

commentaire  = "select distinct 'ZN' Type_commentaire, '', ID_ZN, Date_action, 'ADMIN_IXIN', Commentaire_ZN from infos where ID_ZN is not null and Commentaire_ZN <> '' \
UNION \
select distinct 'CODE IMB' Type_commentaire, code_IMB, '', Date_action, 'ADMIN_IXIN', Commentaire_Qualif  from infos where ID_ZN is null and Commentaire_Qualif <> '' order by 3"


contact_orange_pressyndic = "select ID_ZN, 'Pressyndic' Type_contact, Civilite_President, Prenom_PdtConseilSyndical, Nom_PdtConseilSyndical, N_telephone_pdtconseilsyndical, N_mobile_pdtconseilsyndical, N_fax_pdtconseilsyndical, Email_PdtConseilSyndical, '', '', '', '' from infos where ID_ZN is not null and \
Civilite_President || Prenom_PdtConseilSyndical || Nom_PdtConseilSyndical || N_telephone_pdtconseilsyndical || N_mobile_pdtconseilsyndical || N_fax_pdtconseilsyndical || Email_PdtConseilSyndical <> ''"

contact_orange_gestimm = "select ID_ZN, 'Gestimm' Type_contact, Civilite_Responsable, Prenom_Responsable, Nom_Responsable, N_Tel_syndic, N_Mobile_Syndic, N_Fax_syndic, Email_Syndic, '', '', '', '' from infos where ID_ZN is not null and \
Type_contact || Civilite_Responsable || Prenom_Responsable || Nom_Responsable || N_Tel_syndic || N_Mobile_Syndic || N_Fax_syndic || Email_Syndic"



contact_projet = "select distinct ID_ZN, 'ZN' Type_contact, 'M' civilite, 'Histo' prenom_contact, Nom_contact, '' Ville_Contact,'' CP, '' Code_INSEE, '0' || Tel_contact, '' Tel, '' Fax, Mail_contact  ,Adresse_contact, '' Commentaire from infos where ID_ZN is not null and Nom_contact <> '' and Tel_contact <> ''"


Etudes = "select code_IMB, Adresse, plaque, '' centre, Localite, PB_Geof_Geofibre, Adduction_pb_vers_PA, Type_qualif_ETD, Numero_support_site, Adduction_pb_vers_EL, PT_IPON,  Commentaire_RGT from infos \
where PB_Geof_Geofibre || Adduction_pb_vers_PA || Type_qualif_ETD || Numero_support_site || Adduction_pb_vers_EL || PT_IPON <> ''"
